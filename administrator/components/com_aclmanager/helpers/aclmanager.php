...
class AclmanagerHelper
{
	 ...
	 * Note: Submenu functionality is deprecated in Joomla 4.x
	 * This method is kept for backward compatibility but does nothing.
	 */
	public static function addSubmenu($vName)
	{
		// Submenu functionality removed in Joomla 4.x
		// Navigation is now handled by the main menu system
		return;
	}
	...
	static function getComponentList()
	{
		...
		$query->select('name AS value, name AS text, level AS level, rules AS rules')
			...
			->where($db->quoteName('rules') . ' != ' . $db->quote('{}'));
		...
		if (count($options)) {
			...
			ArrayHelper::sortObjects($options, 'text', 1, true, $lang->getLocale());
		}
	...
	static function assetAction($asset,$action,$group,$user)
	{
		...
		if($group==0) {
			$su = Access::check($user, 'core.admin', 1);
			...
			$su = Access::checkGroup($group, 'core.admin', 1);
	...
	static function asset($assets,$groupid)
	{
		...
		foreach ($assets as $i => $asset) {
			...
			if (in_array($asset->id,$trashed_categories) || in_array($asset->id,$trashed_content)) {
				$asset->status = '<em>['.Text::_('JTRASHED').']</em>';
				$asset->class = 'trashed';
			} else {
				$asset->status = '';
				$asset->class = '';
			...
			// Check for trashed objects
			if(Factory::getUser()->authorise('core.admin', $asset->component)) {
				$asset->config = true;
			} else {
				$asset->config = false;
			}
			...
			if ($asset->name == 'root.1'){
				...
				if ((Factory::getUser()->authorise('core.admin', 'com_config')) && (!$print)) {
					$asset->url =  '<a href="'.Route::_('index.php?option=com_config').'">'.Text::_('COM_CONFIG_GLOBAL_CONFIGURATION').'</a>';
				} else {
					$asset->url =  '<span class="title">'.Text::_('COM_CONFIG_GLOBAL_CONFIGURATION').'</span>';
				}
				$asset->type = 'config';
				$asset->icon = 'config';
			} elseif ($asset->level == 1) {
				// Component level
				if ((Factory::getUser()->authorise('core.manage', $asset->component)) && ($permissions) && (!$print)) {
					$asset->url =  '<a href="'.Route::_('index.php?option='.$asset->name).'">'.Text::_($asset->component).'</a>';
				} elseif ((Factory::getUser()->authorise('core.manage', $asset->component)) && (!$print)) {
					$asset->url =  '<span class="icon-16-aclmanager hasTip" title="'.Text::_('COM_ACLMANAGER_TABLE_ADDED_SUPPORT_TITLE').'::'.Text::_('COM_ACLMANAGER_TABLE_ADDED_SUPPORT_DESC').'"></span><a href="'.Route::_('index.php?option='.$asset->name).'">'.Text::_($asset->component).'</a>';
				} elseif (!$permissions) {
					$asset->url =  '<span class="icon-16-aclmanager hasTip" title="'.Text::_('COM_ACLMANAGER_TABLE_ADDED_SUPPORT_TITLE').'::'.Text::_('COM_ACLMANAGER_TABLE_ADDED_SUPPORT_DESC').'"></span><span class="title">'.Text::_($asset->title).'</span>';
				} else {
					$asset->url =  '<span class="title">'.Text::_($asset->title).'</span>';
				}
				...
				if($acl_categorymanager) {
					if ((Factory::getUser()->authorise('core.manage', $asset->component)) && (Factory::getUser()->authorise('core.manage', 'com_categories')) && (!$print)) {
						$asset->url =  '<a href="'.Route::_('index.php?option=com_categories&task=category.edit&id='.$asset->targetid.'&extension='.$asset->component).'">'.$asset->title.' '.$asset->status.'</a>';
					} else {
						$asset->url =  '<span class="title">'.Text::_($asset->title).' '.$asset->status.'</span>';
					}
				} else {
					if ((Factory::getUser()->authorise('core.manage', $asset->component)) && (!$print)) {
						$asset->url =  '<a href="'.Route::_('index.php?option=com_categories&task=category.edit&id='.$asset->targetid.'&extension='.$asset->component).'">'.$asset->title.' '.$asset->status.'</a>';
					} else {
						$asset->url =  '<span class="title">'.Text::_($asset->title).' '.$asset->status.'</span>';
					}
				...
				// Article level
				if ((Factory::getUser()->authorise('core.manage', $asset->component)) && (!$print)) {
					$asset->url =  '<a href="'.Route::_('index.php?option='.$asset->component.'&task=article.edit&id='.$asset->targetid.'&extension='.$asset->component).'">'.$asset->title.' '.$asset->status.'</a>';
				} else {
					$asset->url =  '<span class="title">'.Text::_($asset->title).' '.$asset->status.'</span>';
				}
				$asset->type	= $types[1];
				...
				// Module type
				if ((Factory::getUser()->authorise('core.manage', $asset->component)) && (!$print)) {
					$asset->url =  '<a href="'.Route::_('index.php?option='.$asset->component.'&task=module.edit&id='.$asset->targetid).'">'.$asset->title.' '.$asset->status.'</a>';
				} else {
					$asset->url =  '<span class="title">'.Text::_($asset->title).' '.$asset->status.'</span>';
				}
				$asset->type	= $types[1];
				...
				// Third Party
				if ((Factory::getUser()->authorise('core.manage', $asset->component)) && (!$print)) {
					$asset->url =  '<a href="'.Route::_('index.php?option='.$asset->component).'">'.$asset->title.'</a>';
				} else {
					$asset->url =  '<span class="title">'.Text::_($asset->title).'</span>';
				}
				$asset->type	= $types[1];
