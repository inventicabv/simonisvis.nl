 ...
 * Aclmanager Model
 */
class AclmanagerModelDiagnostic extends ListModel
{
			...
			foreach ($assets as $asset)
			{
				...
				// Component
					...
					if (in_array($asset->name,$core_noassets,true)) {
						...
						if(is_file($xmlfile)){
							$actions = Access::getActionsFromFile($xmlfile);
							$actionname = array();
				...
				// Category
					...
					if (isset($catinfo)) {
						...
						if($asset->correct_parent == 0) {
							...
							if($view == 'diagnostic') {
								$object_url = Route::_('index.php?option=com_categories&task=category.edit&id='.$asset->object[2].'&extension='.$asset->component, false);
								Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ISSUE_DETECTED_CATEGORY', $object_url, $asset->title), 'error');
							}
						}
					}
					...
					if (($asset->rules == '') || ($asset->rules == '{"core.create":[],"core.delete":[],"core.edit":[],"core.edit.state":[]}') || ($asset->rules == $category_asset)) {
						...
						if(is_file($xmlfile)){
							$actions = Access::getActionsFromFile($xmlfile, "/access/section[@name='category']/");
							$actionname = array();
							foreach($actions as $action) {
								$actionname[] = '"'.$action->name.'":[]';
							}
				...
				// Article
					...
					if($asset->correct_parent == 0) {
						...
						$asset->correct_level = $assets_name[$asset->component]->level + 1;
						if($view == 'diagnostic') {
							$object_url = Route::_('index.php?option=com_content&task=article.edit&id='.$asset->object[2], false);
							Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ISSUE_DETECTED_ARTICLE', $object_url, $asset->title), 'error');
						}
					}
				// Module
	...
	public function fixAssetIssues()
	{
		...
		if($issues) {
			...
			if($total == 1){
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ASSET_ISSUES_FIXED', $total));
			} else {
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ASSET_ISSUES_FIXED_PLURAL', $total));
			}
		}

		$success = $this->rebuild();
		if($success) {
			Factory::getApplication()->enqueueMessage(Text::_('COM_ACLMANAGER_DIAGNOSTIC_REBUILD_SUCCESS'));
		} else {
			Factory::getApplication()->enqueueMessage(Text::_('COM_ACLMANAGER_DIAGNOSTIC_REBUILD_FAILED'), 'error');
		}
	}
	...
	public function fixOrphanAssets()
	{
		...
		if (!$this->getDbo()->execute())
			{
				$this->setError(Text::_('COM_ACLMANAGER_DIAGNOSTIC_ORPHAN_ASSETS_FAILED'));
				return false;
			}

			if($total == 1){
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ORPHAN_ASSETS_FIXED', $total));
			} else {
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ORPHAN_ASSETS_FIXED_PLURAL', $total));
			}
		}
	...
	public function fixMissingAssets()
	{
		...
		if (!$this->getDbo()->execute())
				{
					$this->setError(Text::_('COM_ACLMANAGER_DIAGNOSTIC_MISSING_ASSETS_FAILED'));
					return false;
				}
			...
			if($total == 1){
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_MISSING_ASSETS_FIXED', $total));
			} else {
				Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_MISSING_ASSETS_FIXED_PLURAL', $total));
			}
		}
	...
	public function fixAdminConflicts()
	{
		...
		if (!$db->execute()) {
			$this->setError(Text::_('COM_ACLMANAGER_DIAGNOSTIC_ADMIN_ACCESS_CONFLICTS_FAILED'));
			return false;
		}

		if($total == 1){
			Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ADMIN_ACCESS_CONFLICTS_FIXED', $total));
		} else {
			Factory::getApplication()->enqueueMessage(Text::sprintf('COM_ACLMANAGER_DIAGNOSTIC_ADMIN_ACCESS_CONFLICTS_FIXED_PLURAL', $total));
		}
	}
	...
	public function rebuild($parentId = null, $leftId = 0, $level = 0, $path = '')
	{
		...
		$this->getDbo()->setQuery(sprintf($this->_cache['rebuild.sql'], (int) $parentId));
		$children = $this->getDbo()->loadObjectList();
		...
		if (!$this->getDbo()->execute())
		{
			$this->setError(Text::sprintf('JLIB_DATABASE_ERROR_REBUILD_FAILED', get_class($this), $this->getDbo()->getErrorMsg()));
			return false;
		}
