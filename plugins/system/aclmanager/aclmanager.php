...
class PlgSystemAclmanager extends CMSPlugin
{
	function onAfterRoute() {
		...
		if($app->isClient('administrator')) {
			...
			$jversion 	= str_replace('.', '', $jversion->getShortVersion());
			...
			if($user->id && $extensionfolder && !$permissions && !$core) {
				if (!$user->authorise('core.manage', $option)) {
					$input->set('option', 'com_aclmanager');
					$input->set('view', 'notauthorised');
	...
	function onAfterRender()
	{
		...
		if (($acl_categorymanager) && (!Factory::getUser()->authorise('core.manage', 'com_categories'))) {
			$output = $app->getBody();
	...
	private function loadOriginalClassAsCore($path, $file){
		...
		if ($file = Path::find(JPATH_BASE.$path, $file)) {
			// Read file
